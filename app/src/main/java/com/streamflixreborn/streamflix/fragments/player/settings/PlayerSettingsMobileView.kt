        ...
        fun displaySettings(item: Item) {
            ...
            if (item is Settings.Server) {
                ...
                binding.root.setOnLongClickListener {
                    ...
                    //   l'extracteur à la base, du coup on était sûr que ça revienne pas » ; et
                    //   « quand j'allais dans Gérer les sources je trouvais pas les serveurs, je
                    //   trouvais pas ça normal ») : on vise l'EXTRACTEUR, plus le lien seul. Un lien
                    //   porte un id souvent POSITIONNEL (numéroté par rang dans la liste du
                    //   provider) : il revenait au scrape suivant sous un autre numéro. Un
                    //   extracteur ne peut être ramené par aucune source, et il est recochable dans
                    //   « Gérer les sources ». Repli sur le lien seul quand aucun extracteur ne se
                    //   reconnaît (fichier direct, flux natif d'un provider).
                ...
                // 2026-05-08 : ban IPTV via croix (ivSettingBan) — affiche
                // une petite croix après le nom du server. Click sur croix
                // → toggle ban (persisté). Si banni : alpha 0.4 + ne sera pas
                // joué automatiquement au démarrage de la chaîne.
                val srvChannelKey = item.channelKey
                if (item.isIptv && srvChannelKey != null) {
                    ...
                    // Cœur (♥) — toggle favori (max 5 par chaîne, ordre user)
            ...
            if (item is Settings.ChannelVariant && item.isIptv) {
                ...
                binding.ivSettingFavorite.setOnClickListener {
                    if (item.channelKey.isBlank()) {
                        android.util.Log.w("FavoriteDebug",
                            "channelKey is BLANK — toggleFavorite will no-op!")
                    }
                ...
                // 2026-05-08 : NE PAS reset ivSettingBan/Favorite à GONE si c'est
            ...
            binding.tvSettingMainText.apply {
                ...
                // Use ColorStateList for the default case so focused state can
                // switch text to black on the white focus background — otherwise
