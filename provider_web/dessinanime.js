...
(function () {
  ...
  function chunk(arr, size, names) {
    var out = [];
    for (var i = 0, n = 0; i < arr.length; i += size, n++) out.push({ name: names[n] || (names[0] + ' ' + (n + 1)), items: arr.slice(i, i + size) });
    return out;
  }
  ...
  var P = {
    ...
    getEpisodesBySeason: async function (seasonId) {
      ...
      return sorted.map(function (n) {
        ...
        if (ci >= 0) { var c = big.substring(Math.max(0, ci - 500), ci); var im = c.match(/"src":"(https:\/\/image\.tmdb\.org\/[^"]+)","alt":"([^"]*)"/); if (im) { poster = lightPoster(im[1]); var t = decode(im[2]); if (t && !/^épisode/i.test(t)) title = t; } }
